...
import { Button,useDisclosure, Heading, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay, HStack, Box,Image,Text, VStack } from '@chakra-ui/react';
...
const Shop = () => {
    ...
    const [pop,setPop]=React.useState({image:"https://www.uboric.com/wp-content/uploads/2022/08/12-Piece-Lot-Cotton-Tank-Top-130x130.jpg",name:"12 Piece Lot Cotton Tank Top",price:"10000"})
    ...
    React.useEffect(()=>{
        fetch("http://localhost:5000/clothing").then((res)=>res.json()).then((res)=>setProducts(res))

    },[])
  return (
    <div>
        <div style={{width:"80%",margin:"auto" ,marginTop:"50px",justifyContent:"space-between",display:"flex"}}>
        ...
        <div style={{width:"70%" ,marginBottom:"40px"}}>
            ...
            <div width="100%" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"30px"}}>
                {products.map((p)=><div onClick={()=>onClick(p.name,p.image,p.price)}><SingleItem image={p.image} name={p.name} price={p.price} /></div>)}
            </div>
        </div>
        ...
        <ModalContent  >
          ...
          <ModalBody >
            <HStack height="200px"justify={"space-between"}>
                <HStack width="80%" borderRight={"1px solid gainsboro"} mr="10px">
                    <Box w="50%">
                        <Image w="80%" src={pop.image} alt="pro"/>
                    </Box>
                    <Box >
                        <Heading size="sm" mb="5px">{pop.name}</Heading>
                ...
                <VStack width="30%" >
                    <Box w="100%"><Link to="/cart"><Button colorScheme='blackAlpha' style={{borderRadius:"0px", width:"100%"}}>View Cart</Button></Link></Box>
                    <Box w="100%"><Button isLoading={loading}loadingText="Adding.."colorScheme='blackAlpha' style={{borderRadius:"0px", width:"100%"}} onClick={handleAddCart} >Add To Cart</Button></Box>
                </VStack>
